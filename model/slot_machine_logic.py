...
import time
SLOT_MACHINE_REEL = ["🍒"] * 5 + ["💎"] * 2 + ["7"] * 3   #weighted probability
...
class slot_machine:        
    ...
    def calculate_payout(self, spin_results, user_bet):
        if spin_results.count("🍒") == len(spin_results):
            payout = int(user_bet) * 3 #multiplier of bet when win
            print("YOU WIN x3!")
            return payout

        elif spin_results.count("💎") == len(spin_results):
            payout = int(user_bet) * 100 #multiplier of bet when win
            print("JACK POT!")
            return payout
        
        elif spin_results.count("7") == len(spin_results):
            payout = int(user_bet) * 10 #multiplier of bet when win
            print("YOU WIN x10!")
            return payout
            ...
            payout = int(user_bet) * 5 #multiplier of bet when win
            print("YOU WIN x5!")
            return payout
